...
class HttpTaskManagerTest extends TaskManagerTest<HttpTaskManager> {
    ...
    void shouldLoadSingleTask() {
        ...
        String expectedResult = "SingleTask{id=0, name='First single task for testing', description='Desc AST'," +
                " type='SINGLE', status='NEW', start time ='1970-01-01T00:06:40Z', duration ='8400'}\n";
    ...
    void shouldLoadEpicTask() {
        ...
        String expectedResult = "EpicTask{id=0, name='First epic for testing', description='Desc FE for testing '," +
                " type='EPIC', status=NEW, start time ='null', duration ='0', subtaskList=[]}\n";
    ...
    void shouldLoadSubtask() {
        ...
        String expectedResult = "SubTask{id=0, name='First subtask for testing'," +
                " description='Desc FSB', type='SUBTASK', status=NEW, start time ='1975-03-12T12:05:00Z', " +
                "duration ='707568400', epic id='0'}\n";
    ...
    public void shouldLoadSortedTasks() {
        ...
        taskManager.addSingleTask(singleTaskSecond);
        HttpTaskManager httpTaskManager = new HttpTaskManager("http://localhost:8078", true);
        assertEquals(taskManager.getPrioritizedTasks(),
                httpTaskManager.getPrioritizedTasks(), "lists are not equal");
    }
    ...
    public void shouldLoadSingleTasks() {
        ...
        taskManager.addSingleTask(singleTaskSecond);
        HttpTaskManager httpTaskManager = new HttpTaskManager("http://localhost:8078", true);
        assertEquals(taskManager.getAllSingleTasks(),
                httpTaskManager.getAllSingleTasks(), "lists are not equal");
    }
    ...
    @Test
    @DisplayName("Выгрузка подзадачи")
    public void shouldLoadSubtasks() {
        ...
        taskManager.addNewSubTask(subtaskSecond);
        HttpTaskManager httpTaskManager = new HttpTaskManager("http://localhost:8078", true);
        assertEquals(taskManager.getAllSubtasks(), httpTaskManager.getAllSubtasks(), "lists are not equal");
    }
    ...
    public void shouldHaveTheEqualIds() {
        ...
        taskManager.addSingleTask(singleTaskSecond);
        HttpTaskManager httpTaskManager = new HttpTaskManager("http://localhost:8078", true);
        assertEquals(taskManager.getTaskById(singleTaskSecond.getId()),
                httpTaskManager.getTaskById(singleTaskSecond.getId()), "lists are not equal");
    }
