...
class HttpTaskManagerTest {
    ...
    void beforeEach() throws IOException {
        ...
        httpTaskManager = Managers.getDefaultHttpTaskManager("http://localhost:8078");

        singleTaskOne = new SingleTask("First single task for testing", "Desc AST", TaskStatus.NEW, Instant.ofEpochMilli(400000L), 8400L);
        singleTaskSecond = new SingleTask("First single task for testing", "Desc AST", TaskStatus.NEW, Instant.ofEpochMilli(1099400000L), 10L);
        ...
        subtaskOne = new Subtask("First subtask for testing", "Desc FSB", TaskStatus.NEW, Instant.ofEpochMilli(163857900000L), 707568400L, epicTaskSecond.getId());
        subtaskSecond = new Subtask("Second subtask for testing", "Desc SSB", TaskStatus.NEW, Instant.ofEpochMilli(99173857900000L), 707568400L, epicTaskSecond.getId());
    }
    ...
    void shouldSaveSingleTask() {
        assert httpTaskManager != null;
        httpTaskManager.addSingleTask(singleTaskOne);
        httpTaskManager.addSingleTask(singleTaskSecond);
        int id = singleTaskSecond.getId();
        Task ExpecterResultTask = httpTaskManager.getTaskById(id);
    ...
    void shouldSaveEpicTask() {
        assert httpTaskManager != null;
        httpTaskManager.addEpicTask(epicTaskOne);
        httpTaskManager.addEpicTask(epicTaskSecond);
    ...
    void shouldSaveSubtask() {
        assert httpTaskManager != null;
        httpTaskManager.addEpicTask(epicTaskOne);
        httpTaskManager.addNewSubTask(subtaskOne);
        httpTaskManager.addNewSubTask(subtaskSecond);
    ...
    @Test
    @DisplayName("Выгрузка Истории")
    public void shouldLoadTasksHistory() {
        ...
        taskManager.getTaskById(singleTaskSecond.getId());
        httpTaskManager.addSingleTask(singleTaskOne);
        httpTaskManager.addSingleTask(singleTaskSecond);
        httpTaskManager.getTaskById(singleTaskOne.getId());
        httpTaskManager.getTaskById(singleTaskSecond.getId());
        assertEquals(taskManager.getHistory(), httpTaskManager.getHistory(), "lists are not equal");
    }

    @Test
    @DisplayName("Выгрузка отсортированного списка")
    public void shouldLoadSortedTasks() {
        TaskManager taskManager = Managers.getDefault();
        taskManager.addSingleTask(singleTaskOne);
        taskManager.addSingleTask(singleTaskSecond);
        httpTaskManager.addSingleTask(singleTaskOne);
        httpTaskManager.addSingleTask(singleTaskSecond);
        assertEquals(taskManager.getPrioritizedTasks(), httpTaskManager.getPrioritizedTasks(), "lists are not equal");
    }

    @Test
    @DisplayName("Выгрузка задач")
    public void shouldLoadSingleTasks() {
        TaskManager taskManager = Managers.getDefault();
        taskManager.addSingleTask(singleTaskOne);
        taskManager.addSingleTask(singleTaskSecond);
        httpTaskManager.addSingleTask(singleTaskOne);
        httpTaskManager.addSingleTask(singleTaskSecond);
        assertEquals(taskManager.getAllSingleTasks(), httpTaskManager.getAllSingleTasks(), "lists are not equal");
    }

    @Test
    @DisplayName("Выгрузка Эпика")
    public void shouldLoadEpicTasks() {
        TaskManager taskManager = Managers.getDefault();
        taskManager.addEpicTask(epicTaskOne);
        taskManager.addEpicTask(epicTaskSecond);
        httpTaskManager.addEpicTask(epicTaskOne);
        httpTaskManager.addEpicTask(epicTaskSecond);
        assertEquals(taskManager.getAllEpicTasks(), httpTaskManager.getAllEpicTasks(), "lists are not equal");
    }

    @Test
    @DisplayName("Проверка Id")
    public void shouldHaveTheEqualIds() {
        TaskManager taskManager = Managers.getDefault();
        taskManager.addSingleTask(singleTaskOne);
        taskManager.addSingleTask(singleTaskSecond);
        httpTaskManager.addSingleTask(singleTaskOne);
        httpTaskManager.addSingleTask(singleTaskSecond);
        assertEquals(taskManager.getTaskById(singleTaskSecond.getId()), httpTaskManager.getTaskById(singleTaskSecond.getId()), "lists are not equal");
    }
}
